...
export async function handler(event: any) {
  ...
  const systemPrompt = [
    ...
    '- 3枚目(相談)には「よくある相談」として2〜4個のbulletsを必ず入れてください。5枚目(具体例)にはbulletsを入れてもよいです。7枚目(行動提案)はその回の内容を一旦完結させるページにしてください。7枚目には公式LINE、プロフィール、リンク、無料診断、登録などのCTA表現を入れないでください。CTAは8枚目でアプリ側が別ページとして作ります。',
